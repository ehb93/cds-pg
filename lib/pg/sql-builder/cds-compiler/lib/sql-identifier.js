...
function smartId(name, dialect) {
  const s = typeof dialect === "string" ? sqlDialects[dialect] : dialect;
  console.log("Tenemos smartid: " + name);
  console.log("Tenemos smartid dialect: " + dialect);
  try {
    if (
      (s.regularRegex && !s.regularRegex.test(name)) ||
